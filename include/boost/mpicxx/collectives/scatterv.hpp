...
scatterv(const communicator& comm, const std::vector<T>& in_values, 
         ...
         T* out_values, int out_size, int root)
{
  ::boost::mpicxx::scatterv(comm, in_values.data(), sizes, displs,
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         const std::vector<int>& sizes, T* out_values, int root)
{
  ::boost::mpicxx::scatterv(comm, &in_values[0], sizes, out_values, root);
}
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         T* out_values, int out_size, int root)
{
  ::boost::mpicxx::scatterv(comm, &in_values[0], out_values, out_size, root);
}
